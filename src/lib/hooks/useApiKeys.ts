...
import type { ApiKey, ApiKeyUsage } from '@/lib/types/api.types';
...
export function useCreateApiKey(token: string | null) {
  ...
  return useMutation({
    mutationFn: (data: { name: string; scopes: string[]; expires_at?: string; allowed_ips?: string[] | null; blocked_ips?: string[] | null }) =>
...
export function useUpdateApiKey(token: string | null) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...patch }: { id: number; allowed_ips?: string[] | null; blocked_ips?: string[] | null; is_active?: boolean }) =>
...
export function useApiKeyUsage(token: string | null, id: number | null) {
  return useQuery({
    queryKey: ['api-key-usage', id, token],
    queryFn: async () => {
      const res = await customAxiosGet(`${base}/api-keys/${id}/usage`, undefined, token ?? undefined);
      return res.status === true ? (res.response as ApiKeyUsage) : null;
    },
    enabled: id != null,
  });
}
